...
import { investment } from "@/server/services/analytics";
import { jsonSafe } from "@/lib/bigint-json";

type Ctx = { params: Promise<{ id: string }> };

// spec-014: GET /api/v1/projects/:id/analytics/investment
export const GET = apiHandler<Ctx>(async (request, ctx) => {
  ...
  return Response.json(jsonSafe(await investment(Number(id))));
});
